...
const TeachersNotes = () => {
  ...
  const fetchTeachersNotes = async () => {
    try {
      ...
      setError(null);
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/notes/teachers', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchSubjectsAndSemesters = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/subjects-semesters', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchStudyFolders = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/study-folders', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchSummaryNotes = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/note-summaries/summaries', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const generateAutoSummary = async (noteId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/note-summaries/auto-summary', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSaveSubmit = async () => {
    try {
      ...
      if (showNewFolderInput && newFolder.trim()) {
        const token = localStorage.getItem('token');
        const createFolderResponse = await fetch('http://localhost:5000/api/study-folders', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      // Save note to folder
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/study-folders/add-note', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
            ...
            <div className="note-preview">
              {selectedNote.file_type === 'pdf' && (
                <div className="pdf-viewer">
                  <iframe
                    src={`http://localhost:5000${selectedNote.file_path}`}
