...
const Dashboard = () => {
  ...
  //       "http://localhost:5000/api/user/subscription-info",
  //       {
  //         headers: {
  ...
  const fetchNewNotesCount = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/dashboard/new-notes-count",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleTeacherNotesClick = async () => {
    try {
      ...
      await fetch("http://localhost:5000/api/dashboard/mark-notes-viewed", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (data?.role === "teacher") {
        ...
        if (teacherResponse.ok) {
          ...
          if (teacherData.photo_url) {
            setProfilePhoto(`http://localhost:5000${teacherData.photo_url}`);
          }
        }
      } else if (data?.photoUrl) {
        // For students
        setProfilePhoto(`http://localhost:5000${data.photoUrl}`);
      }
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const teachersResponse = await fetch(
        "http://localhost:5000/api/dashboard/teachers-notes-stats",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const studentsResponse = await fetch(
        "http://localhost:5000/api/dashboard/total-students",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const teachersResponse2 = await fetch(
        "http://localhost:5000/api/dashboard/total-teachers",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const studentsByDepartmentResponse = await fetch(
        "http://localhost:5000/api/dashboard/students-by-department",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
