...
const TechPrepLearning = () => {
  ...
  const fetchSkillDetails = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/tech-prep/skills/${skillId}/details`, {
        headers: {
  ...
  const renderContent = (contentItem) => {
    switch (contentItem.content_type) {
      case 'pdf':
        return (
          <iframe
            src={`http://localhost:5000${contentItem.file_path}`}
            width="100%"
            height="600px"
      ...
      case 'video':
        return (
          <video controls width="100%" height="400px">
            <source src={`http://localhost:5000${contentItem.file_path}`} type="video/mp4" />
